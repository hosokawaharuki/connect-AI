...
function initWhiteboard() {
    ...
    document.getElementById('btn-zoom-in').onclick = () => { scale = Math.min(4.0, scale + 0.2); updateTransform(); };
    ...
    function createLayer(name = `レイヤー ${layers.length + 1}`) {
        ...
        canvas.height = boardHeight;
        layersStack.appendChild(canvas);
        const layerObj = { id, name, canvas, ctx: canvas.getContext('2d') };
    ...
    function updateLayerUI() {
        ...
        layers.slice().reverse().forEach(l => {
            ...
            item.innerHTML = `<span class="layer-name">${l.name}</span>`;
    ...
    firstCanvas.addEventListener('mousedown', (e) => {
        ...
        lastMid = { x: smoothedX, y: smoothedY };
    ...
    window.addEventListener('mouseup', () => {
        if (drawing) {
            if (points.length > 0) {
                ...
                localDraftCtx.moveTo(lastMid.x, lastMid.y);
                localDraftCtx.lineTo(pLast.x, pLast.y);
                localDraftCtx.stroke();
                socket.emit('draw', { state: 'draw', user: currentUsername, x0: lastMid.x, y0: lastMid.y, x1: pLast.x, y1: pLast.y, color: penColor, size: localDraftCtx.lineWidth, eraser: isEraser, isCurve: false });
            }
    ...
    firstCanvas.addEventListener('mousemove', (e) => {
        ...
        const currentMid = { x: (p2.x + p3.x) / 2, y: (p2.y + p3.y) / 2 };

        const penType = document.getElementById('pen-type').value;
        const bgColor = document.getElementById('bg-color-picker').value;
        
        localDraftCtx.strokeStyle = isEraser ? bgColor : penColor;
        localDraftCtx.fillStyle = isEraser ? bgColor : penColor;
        localDraftCtx.globalAlpha = 1.0;
        localDraftCtx.lineCap = 'round';
        localDraftCtx.lineJoin = 'round';

        let targetWidth = penSize;
        if (penType === 'gpen' && !isEraser) {
            const p1 = points[Math.max(0, points.length - 3)];
            const dist = Math.hypot(p3.x - p1.x, p3.y - p1.y);
            const dt = Math.max(1, p3.time - p1.time);
            const speed = dist / dt;
            
            targetWidth = Math.max(0.5, penSize * Math.min(2.0, Math.max(0.1, 2.0 - speed * 0.8)));
        }
        
        currentWidth = currentWidth + (targetWidth - currentWidth) * 0.08;
        localDraftCtx.lineWidth = currentWidth;

        localDraftCtx.beginPath();
        localDraftCtx.moveTo(lastMid.x, lastMid.y);
        localDraftCtx.quadraticCurveTo(p2.x, p2.y, currentMid.x, currentMid.y);
        localDraftCtx.stroke();
        localDraftCtx.closePath();

        socket.emit('draw', { 
            state: 'draw', user: currentUsername,
            x0: lastMid.x, y0: lastMid.y, cx: p2.x, cy: p2.y, x1: currentMid.x, y1: currentMid.y, 
            color: penColor, size: localDraftCtx.lineWidth, eraser: isEraser, 
            isCurve: true
        });
    ...
    socket.on('draw_sync', (data) => {
        if (data.state === 'start') {
            ...
            remote.ctx.strokeStyle = data.eraser ? bgColor : data.color;
            remote.ctx.lineWidth = data.size;
            remote.ctx.globalAlpha = 1.0;
            ...
            if (data.isCurve) {
                remote.ctx.quadraticCurveTo(data.cx, data.cy, data.x1, data.y1);
            } else {
    ...
    peerConnection.ontrack = (event) => {
        ...
        if (remoteVideo.srcObject !== event.streams[0]) {
            ...
            if (playPromise !== undefined) {
                playPromise.catch(e => console.error("リモート音声再生エラー:", e));
            }
        }
...
function initChat() {
    ...
    const sendMessageAction = () => {
        ...
        if(txt) {
            ...
            chatInput.value = '';
            chatInput.style.height = 'auto'; // 送信後に入力欄の高さをリセット
        }
    };

    document.getElementById('btn-send-chat').onclick = sendMessageAction;

    // テキストエリアでの自動高さ調整とEnter/Shift+Enterの制御
    chatInput.addEventListener('input', function() {
        this.style.height = 'auto';
        this.style.height = Math.min(this.scrollHeight, 120) + 'px';
    });

    // Enterで改行、Ctrl+Enter（またはShift+Enter）で送信にするか、あるいは通常のテキストエリアの挙動（Enterで改行）にする
    ...
    document.getElementById('btn-ai-consult').onclick = () => {
        ...
        if (txt) {
            ...
            const myMessages = document.querySelectorAll('.chat-msg.mine .chat-bubble span');
            if (myMessages.length > 0) {
                txt = myMessages[myMessages.length - 1].innerText;
            } else {
                txt = "こんにちは！"; 
            }
        }
        ...
        aiTimeoutTimer = setTimeout(() => {
            ...
            if (indicator) {
                indicator.remove();
                chatMsgs.innerHTML += `
                    <div class="chat-msg other" id="ai-timeout-msg">
                        <span class="chat-sender">🤖 AIアドバイザー</span>
                        <div class="chat-bubble-container">
                            <div class="chat-bubble" style="color: #ff9800;">応答に時間がかかっています。もうしばらくお待ちください...</div>
                        </div>
                    </div>`;
                chatMsgs.scrollTop = chatMsgs.scrollHeight;
            }
    ...
    socket.on('receive_message', (data) => {
        ...
        const timeoutMsg = document.getElementById('ai-timeout-msg');
        if (timeoutMsg && data.user === '🤖 AIアドバイザー') {
            timeoutMsg.remove();
        }
        ...
        let deleteBtnHTML = isMine ? `<button onclick="deleteMessage(${data.id})" style="background:none; border:none; color:#ff5722; font-size:10px; cursor:pointer; padding:0; margin-left:6px;" title="削除">🗑️ 削除</button>` : '';
